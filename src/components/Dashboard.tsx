...
import { ANALYTICS, CYPRESS_BROWSER, DISPLAY, FORM_VALUE, MEDIA_TYPE, REPORT_STATUS, RISK, RUN_DEFAULTS, RUN_LIMITS } from "@/lib/domain-constants";
...
function Distribution({ data }: { data: DashboardData }) {
  const groups = [[RISK.PERSISTENT, data.metrics.persistent, "bg-destructive"], [RISK.HIGH, data.metrics.highRisk, "bg-amber-600"], [RISK.INTERMITTENT, data.metrics.intermittent, "bg-sky-600"], [RISK.ISOLATED, data.metrics.isolated, "bg-emerald-600"]] as const;
  const maximum = Math.max(1, ...groups.map(([, count]) => count));
  return <Card><CardHeader><CardTitle className="flex items-center gap-1">Risk distribution<HelpTip label="About risk distribution">Groups current failures by how often they failed across the selected history window.</HelpTip></CardTitle><CardDescription>Failure frequency across returned history.</CardDescription></CardHeader><CardContent className="space-y-6">{groups.map(([label, count, color]) => <div key={label}><div className="flex justify-between text-sm"><span>{label}</span><span className="font-medium tabular-nums">{count}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-secondary"><div className={`h-full rounded-full ${color}`} style={{ width: `${count / maximum * ANALYTICS.PERCENT_MULTIPLIER}%` }} /></div></div>)}</CardContent></Card>;
}
...
export default function Dashboard({ initialData, reportSelection, reportSourceOptions, sourceRepository, cypressProfiles, reportFields, cypressConfigFields, suggestedProfileId, runner, runnerConfigured, user, localMode }: {
