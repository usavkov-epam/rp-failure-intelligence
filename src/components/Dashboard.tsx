...
import { Activity, Bug, CircleAlert, Clipboard, History, Loader2, Play, Settings2, Zap, type LucideIcon } from "lucide-react";
...
function Metric({ label, value, detail, icon: Icon }: { label: string; value: string; detail: string; icon: LucideIcon }) {
  return <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">{label}</CardTitle><Icon className="size-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold tracking-tight">{value}</div><p className="mt-1 text-xs text-muted-foreground">{detail}</p></CardContent></Card>;
}
  ...
  return <>
    ...
    <main className="pb-16">
      <div className="mx-auto max-w-[1600px] space-y-6 px-4 py-8 lg:px-8">
        ...
        <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>Report source</CardTitle><CardDescription className="mt-2">Project <strong>{reportSelection.project}</strong> is your global workspace context.</CardDescription></div><Button asChild variant="outline" size="sm"><Link href="/settings"><Settings2 />Configure fields</Link></Button></div></CardHeader><CardContent>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={Activity} label="Current suite failure rate" value={`${initialData.metrics.suiteFailureRate.toFixed(1)}%`} detail={`${initialData.metrics.suiteFailed} failed of ${initialData.metrics.suiteTotal} filtered tests`} /><Metric icon={Bug} label="Failed test identities" value={String(initialData.rows.length)} detail={`${new Set(initialData.rows.map((row) => row.specPath)).size} unique Cypress specs`} /><Metric icon={History} label="Historical cohort failures" value={`${initialData.metrics.cohortExecutions ? Math.round(initialData.metrics.cohortFailures / initialData.metrics.cohortExecutions * 100) : 0}%`} detail={`${initialData.metrics.cohortFailures} of ${initialData.metrics.cohortExecutions} observations`} /><Metric icon={Zap} label="Immediate regressions" value={String(initialData.metrics.regressions)} detail="Current failures preceded by a passed run" /></div>
