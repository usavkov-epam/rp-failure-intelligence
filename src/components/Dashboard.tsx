...
import { EMPTY_ANALYSIS_TABLE_FILTERS, getAnalysisTableFiltersSnapshot, parseAnalysisTableFilters, saveAnalysisTableFilters, subscribeToAnalysisSession } from "@/lib/analysis-session";
import { ANALYTICS, CYPRESS_BROWSER, DISPLAY, FORM_VALUE, MEDIA_TYPE, REPORT_STATUS, RISK, RUN_DEFAULTS, RUN_LIMITS, type SpecPathCopyFormat } from "@/lib/domain-constants";
import { RISK_BACKGROUND_CLASS } from "@/lib/risk-presentation";
import { formatSpecPaths } from "@/lib/spec-path-copy";
...
interface CypressRunFormOptions { runs: number; threads: number; browser: typeof CYPRESS_BROWSER[keyof typeof CYPRESS_BROWSER]; timeoutSeconds: number; profileId: string; cypressConfig: CypressConfigOverrides }
const risks: Risk[] = [RISK.PERSISTENT, RISK.HIGH, RISK.INTERMITTENT, RISK.ISOLATED];
...
export default function Dashboard({ initialData, reportSelection, reportSourceOptions, sourceRepository, cypressProfiles, reportFields, cypressConfigFields, specPathCopyFormat, suggestedProfileId, runner, runnerConfigured, user, localMode }: {
  ...
  specPathCopyFormat: SpecPathCopyFormat;
  ...
  const filterSnapshot = useSyncExternalStore(subscribeToAnalysisSession, getAnalysisTableFiltersSnapshot, () => "");
  const filters = useMemo(() => parseAnalysisTableFilters(filterSnapshot) || EMPTY_ANALYSIS_TABLE_FILTERS, [filterSnapshot]);
  const { search, risk, moduleName, classification, failureRate, streak, transitions } = filters;
  const setSearch = (value: string) => saveAnalysisTableFilters({ ...filters, search: value });
  const setRisk = (value: string) => saveAnalysisTableFilters({ ...filters, risk: value });
  const setModuleName = (value: string) => saveAnalysisTableFilters({ ...filters, moduleName: value });
  const setClassification = (value: string) => saveAnalysisTableFilters({ ...filters, classification: value });
  const setFailureRate = (value: string) => saveAnalysisTableFilters({ ...filters, failureRate: value });
  const setStreak = (value: string) => saveAnalysisTableFilters({ ...filters, streak: value });
  const setTransitions = (value: string) => saveAnalysisTableFilters({ ...filters, transitions: value });
  ...
  return <>
    ...
    <main className="pb-16">
      <div className="mx-auto max-w-[1600px] space-y-6 px-4 py-8 lg:px-8">
        ...
        <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="flex items-center gap-1">Report source<HelpTip label="About report source">Select the launch, run, custom ReportPortal filters, and history depth used by every analysis block below.</HelpTip></CardTitle><CardDescription className="mt-2">Project <strong>{reportSelection.project}</strong> is your global workspace context.</CardDescription></div><Button asChild variant="outline" size="sm"><Link href="/settings"><Settings2 />Configure fields</Link></Button></div></CardHeader><CardContent>
          <form action="/" method="get" className="space-y-4">
            <div className="grid items-end gap-4 md:grid-cols-2 lg:grid-cols-[minmax(0,2fr)_minmax(12rem,1fr)_minmax(10rem,0.75fr)]">
              ...
              <FormField label="History depth"><Select name="historyDepth" defaultValue={String(reportSelection.historyDepth)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{DISPLAY.HISTORY_DEPTH_OPTIONS.map((value) => <SelectItem key={value} value={String(value)}>{value} runs</SelectItem>)}</SelectContent></Select></FormField>
            </div>
            {reportFields.length > 0 && <div className="grid items-end gap-4 md:grid-cols-2 xl:grid-cols-4">
              {reportFields.map((field) => <FormField key={field.key} label={field.label}>{field.type === "enum" ? <Select name={`field.${field.key}`} defaultValue={reportSelection.fields[field.key] || (field.required ? undefined : FORM_VALUE.REPORT_ANY)} required={field.required}><SelectTrigger className="w-full"><SelectValue placeholder={field.required ? "Select value" : "Any value"} /></SelectTrigger><SelectContent>{!field.required && <SelectItem value={FORM_VALUE.REPORT_ANY}>Any value</SelectItem>}{field.options.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select> : <Input name={`field.${field.key}`} defaultValue={reportSelection.fields[field.key] || ""} required={field.required} />}</FormField>)}
            </div>}
        ...
        <section className="space-y-4"><div className="flex items-center gap-1"><h2 className="text-lg font-semibold">Failed tests</h2><HelpTip label="About failed tests">Filter every value-bearing column, sort results, select specs, copy paths, and start focused Cypress runs.</HelpTip></div><div className="flex flex-wrap items-end gap-3 border-b pb-4"><FormField label="Search" className="w-full sm:w-72"><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter tests or specs..." /></FormField><FormField label="Risk"><Select value={risk || FORM_VALUE.ANY} onValueChange={(value) => setRisk(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>All risks</SelectItem>{risks.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></FormField><FormField label="Module"><Select value={moduleName || FORM_VALUE.ANY} onValueChange={(value) => setModuleName(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>All modules</SelectItem>{modules.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></FormField><FormField label="Failure rate"><Select value={failureRate || FORM_VALUE.ANY} onValueChange={(value) => setFailureRate(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-36"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>Any rate</SelectItem>{failureRates.map((value) => <SelectItem key={value} value={String(value)}>{value}%</SelectItem>)}</SelectContent></Select></FormField><FormField label="Streak"><Select value={streak || FORM_VALUE.ANY} onValueChange={(value) => setStreak(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-28"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>Any</SelectItem>{streaks.map((value) => <SelectItem key={value} value={String(value)}>{value}</SelectItem>)}</SelectContent></Select></FormField><FormField label="Transitions"><Select value={transitions || FORM_VALUE.ANY} onValueChange={(value) => setTransitions(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-28"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>Any</SelectItem>{transitionCounts.map((value) => <SelectItem key={value} value={String(value)}>{value}</SelectItem>)}</SelectContent></Select></FormField><FormField label="Classification"><Select value={classification || FORM_VALUE.ANY} onValueChange={(value) => setClassification(value === FORM_VALUE.ANY ? "" : value)}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={FORM_VALUE.ANY}>All classifications</SelectItem>{classifications.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></FormField>{(search || risk || moduleName || classification || failureRate || streak || transitions) && <Button variant="ghost" onClick={() => saveAnalysisTableFilters(EMPTY_ANALYSIS_TABLE_FILTERS)}><X />Reset</Button>}<div className="ml-auto flex gap-2"><Button variant="outline" disabled={!selectedSpecs.length} onClick={async () => { await navigator.clipboard.writeText(formatSpecPaths(selectedSpecs, specPathCopyFormat)); toast.success(`${selectedSpecs.length} spec path${selectedSpecs.length === 1 ? "" : "s"} copied`); }}><Clipboard />Copy {selectedSpecs.length || "selected"}</Button><Button disabled={!selectedSpecs.length || !runnerConfigured} title={runnerConfigured ? undefined : "Configure GitHub in Settings before starting a run"} onClick={() => { setRunError(""); setRunDialogOpen(true); }}><Play />Run selected</Button></div></div>{!runnerConfigured && <Alert><CircleAlert /><AlertTitle>Test runner is not configured</AlertTitle><AlertDescription>Connect GitHub in Settings before starting a Cypress run.</AlertDescription></Alert>}<FailureTable rows={rows} historyDepth={initialData.meta.historyDepth} sourceRepository={sourceRepository} onSelectedSpecs={handleSelectedSpecs} /></section>
      </div>
    </main>

    <Dialog open={runDialogOpen} onOpenChange={(open) => !runPending && setRunDialogOpen(open)}><DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Run selected Cypress specs</DialogTitle><DialogDescription>{selectedSpecs.length} unique {selectedSpecs.length === 1 ? "spec" : "specs"}. {runner.executionDescription}</DialogDescription></DialogHeader><div className="space-y-4">
